...
void flota::editaBuque(Puertos & listaPuertos) {
   ...
   if(idBq == 'A' || idBq == 'B' || idBq == 'C' || idBq == 'D' || idBq == 'E') {
     ...
     if(resp == 'S') {
        ...
        vectorBuques[idB].cntOper = 0;   /* Inicializa el contador de operaciones del Buque. */
...
/*DEBUG*/
   ...
   if(idBqC == 'A' || idBqC == 'B' || idBqC == 'C' || idBqC == 'D' || idBqC == 'E') {
     ...
     idP = vectorBuques[iBuque].oprdelBuque[ultOper].puertoI-1;
            ...
            * ------------------------------------------- */
              ...
              for(int k = 0; k < numPuertos; k++) {
                 if(listaPuertos.lista[k].id != 0) {
                    ...
                    if(listaPuertos.lista[k].id != idP) {
                       printf("     %d-", listaPuertos.lista[k].id);
...
/*DEBUG*/
    //fecUltenDias = calBuque.diferenciaFechas(fecha2, fecha1);
    //printf("Fecha de inicio de ultima Operacion:%d\n", fecUltenDias);
    //fecUltenDias = fecUltenDias + vectorBuques[iBuque].oprdelBuque[ultOper].duraC;
    //printf("Fecha de conclusion de ultima Operacion en dias:%d\n", fecUltenDias);
    /* Calculating years */
    //years = fecUltenDias / 365;
    // Calculating months
    //months = (fecUltenDias - (years*365)) / 30;
    // Calculating days
    //days = (fecUltenDias - (years*365) - (months*30));
    //printf("Expresado en Fecha: %2d-%2d-%4d\n", days, months, years);
    //fflush(stdin);
    //getchar();
/*DEBUG*/


     /* Comprueba que la fecha solicitada por el usuario sea mayor o igual a la fecha de la
        ultima Operacion registrada para el Buque.                                           */
     if(calBuque.diferenciaFechas(fecha1, fecha2) < 0) {
        printf("\n\nDebe introducir una fecha igual o posterior a la ultima Operacion del Buque. ENTER para regresar al menu...");
        fflush(stdin);
        getchar();
        ...
        if(vectorBuques[iBuque].oprdelBuque[ultOper].puertoI && !vectorBuques[iBuque].oprdelBuque[ultOper].puertoD && vectorBuques[iBuque].oprdelBuque[ultOper].carga == vacio) {
           /* El Buque esta vacio y anclado en Puerto. */

           idP = vectorBuques[iBuque].oprdelBuque[ultOper].puertoI-1;
           strncpy(puertoAct, listaPuertos.lista[idP].nomP, 10);
           printf("El buque %s esta vacio en %s\n", nombre, puertoAct);

        }else if(vectorBuques[iBuque].oprdelBuque[ultOper].duraC && !statBuque) {
           /* La Operacion mas reciente es de Carga y ha concluido dicha Operacion. */

           /* Determinar el Puerto donde se encuentra el Buque. */
           idP = vectorBuques[iBuque].oprdelBuque[ultOper].puertoI-1;
           strncpy(puertoAct, listaPuertos.lista[idP].nomP, 10);
           printf("\nEl buque %s esta cargado con ", nombre);

           switch(tipoCg) {
              case crudo:    printf("crudo");
                             break;
              case fuel:     printf("fuel");
                             break;
              case gasoil:   printf("gasoil");
                             break;
              case gasolina: printf("gasolina");
                             break;
              case vacio:    printf("vacio");
                             break;
           }
           printf(" en %s\n", puertoAct);
...
/*DEBUG*/
    //ultOper = vectorBuques[iBuque].cntOper-1;
    //printf("\n\nRegistro nuevo:\n");
    //printf("Carga:%d\n", vectorBuques[iBuque].oprdelBuque[ultOper].carga);
    //printf("Puerto Origen:%d\n", vectorBuques[iBuque].oprdelBuque[ultOper].puertoI);
    //printf("Puerto Destino:%d\n", vectorBuques[iBuque].oprdelBuque[ultOper].puertoD);
    //printf("Status de la ultima Operacion:%d", vectorBuques[iBuque].oprdelBuque[ultOper].status);
    //fflush(stdin);
    //getchar();
/*DEBUG*/
           }
        }else if(vectorBuques[iBuque].oprdelBuque[ultOper].duraT && !statBuque) {
           /* La Operacion mas reciente es de Traslado y ha finalizado dicha Operacion. */

           ultOper = vectorBuques[iBuque].cntOper-1;
           printf("\n\nEl Buque se aproxima a Puerto con,\n");
           printf("Carga:%d\n", vectorBuques[iBuque].oprdelBuque[ultOper].carga);
           printf("Puerto Origen:%d\n", vectorBuques[iBuque].oprdelBuque[ultOper].puertoI);
           printf("Puerto Destino:%d\n", vectorBuques[iBuque].oprdelBuque[ultOper].puertoD);
           printf("Presione ENTER para declarar el arribo...");
           fflush(stdin);
           getchar();

           /* IMPORTANTE: Ahora se crea un registro nuevo de Operacion para simular la llegada
              del Buque al Puerto destino. */
           daux = vectorBuques[iBuque].oprdelBuque[ultOper].diaF;
           maux = vectorBuques[iBuque].oprdelBuque[ultOper].mesF;
           aaux = vectorBuques[iBuque].oprdelBuque[ultOper].anoF;

           ultOper++;
           idP = vectorBuques[iBuque].oprdelBuque[ultOper].puertoD;
           vectorBuques[iBuque].oprdelBuque[ultOper].asignaOperacion(tipoCg, idP, 0, 0, 0, 0, daux, maux, aaux, 0);
           vectorBuques[iBuque].cntOper++;

           idP = vectorBuques[iBuque].oprdelBuque[ultOper].puertoD-1;
           strncpy(puertoAct, listaPuertos.lista[idP].nomP, 10);
           printf("El Buque arribo al Puerto %s\n\n", puertoAct);
           printf("Quiere realizar la descarga (S/N)?");
           fflush(stdin);
           resp = toupper(getchar());

/* OJOOOOOO!!! Ver el caso cuando el usuario dice NO a esta opcion. Que ocurre cuando ejecute
   de nuevo la opcion "Operar Buque"???? */

           if(resp == 'S') {

           }
        } else if(vectorBuques[iBuque].oprdelBuque[ultOper].duraD && !statBuque) {
           /* La Operacion mas reciente es de Descarga y ha finalizado dicha Operacion. */
        }else {
        }
     }
   }else {
...
void flota::iniciaFlota() {
   ...
   vectorBuques[2].oprdelBuque[0].asignaOperacion(vacio, 3, 0, 0, 0, 0, 26, 12, 2022, 0);
